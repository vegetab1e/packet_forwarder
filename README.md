...
Макрос `SLOW_MOTION` стоит использовать при работе с `libpcap-base PMD`. К сожалению, ни `uio_pci_generic`, ни `igb_uio` (и такой https://git.dpdk.org/dpdk-kmods и такой https://packages.debian.org/sid/dpdk-kmods-dkms), ни `vfio-pci` с моим оборудованием не работают, поэтому выбора у меня нет. Также с `libpcap-base PMD` форвардер лучше работает, когда **используются страницы памяти по 2 мегабайта в количестве около 2^12**:
